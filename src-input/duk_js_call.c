 ...
 *      Duktape/C function, from native code or bytecode executor.
 *    - duk_handle_safe_call(): protected C call within current activation.
 *
 *  handle_call_unprotected() also handles Ecma-to-Ecma calls where native
 *  recursion depth doesn't increase.  Instead, call setup is done normally
 *  but just before we're ready to call the bytecode executor, the currently
 *  running executor is used to resume execution from the new activation.
 *  The call also handles setup for a resume call.
 *
 *  See 'execution.rst'.
 *
...
DUK_LOCAL_DECL duk_int_t duk__handle_call_raw(duk_hthread *thr,
                                              duk_idx_t idx_func,
...
DUK_LOCAL_DECL void duk__handle_safe_call_shared_unwind(duk_hthread *thr,
                                                        duk_idx_t idx_retbase,
                                                        duk_idx_t num_stack_rets,
#if defined(DUK_USE_ASSERTIONS)
                                                        duk_size_t entry_callstack_top,
...
DUK_LOCAL void duk__safe_call_adjust_valstack(duk_hthread *thr, duk_idx_t idx_retbase, duk_idx_t num_stack_rets, duk_idx_t num_actual_rets) {
	...
	if (idx_rcbase > idx_retbase) {
		 ...
		 * return values to idx_retbase.  Because of valstack
		 * init policy this is just a valstack_top update.
		 */
...
DUK_LOCAL duk_small_uint_t duk__call_setup_act_attempt_tailcall(duk_hthread *thr, duk_small_uint_t call_flags, duk_idx_t idx_func, duk_hobject *func, duk_size_t entry_valstack_bottom_byteoff, duk_idx_t *out_nargs, duk_idx_t *out_nregs, duk_size_t *out_vs_min_bytes, duk_activation **out_act) {
	duk_activation *act;
	duk_tval *tv1, *tv2;
	duk_idx_t idx_args;
	duk_size_t vs_min_regs;
	...
	*out_act = act;
	...
	if (func == NULL || !DUK_HOBJECT_IS_COMPFUNC(func)) {
		DUK_D(DUK_DPRINT("tail call prevented by target not being ecma function"));
		return 0;
...
#error incorrect options: tail calls enabled with function caller property
	...
	*out_nregs = ((duk_hcompfunc *) func)->nregs;
	...
	vs_min_regs = *out_nregs + DUK_VALSTACK_INTERNAL_EXTRA;
	*out_vs_min_bytes = entry_valstack_bottom_byteoff + sizeof(duk_tval) * vs_min_regs;
...
DUK_LOCAL void duk__call_setup_act_not_tailcall(duk_hthread *thr, duk_small_uint_t call_flags, duk_idx_t num_stack_args, duk_idx_t idx_func, duk_hobject *func, duk_size_t entry_valstack_bottom_byteoff, duk_idx_t *out_nargs, duk_idx_t *out_nregs, duk_size_t *out_vs_min_bytes, duk_activation **out_act) {
	duk_activation *act;
...
#if 0  /*FIXME: flag can be removed*/
	...
	*out_act = act;
	...
	if (DUK_LIKELY(func != NULL)) {
		...
		if (DUK_HOBJECT_IS_COMPFUNC(func)) {
			...
			*out_nregs = *out_nargs;
			if (*out_nargs >= 0) {
				vs_min_regs = *out_nregs + DUK_VALSTACK_API_ENTRY_MINIMUM + DUK_VALSTACK_INTERNAL_EXTRA;
			} else {
		...
		*out_nargs = DUK_LFUNC_FLAGS_GET_NARGS(lf_flags);
		if (*out_nargs != DUK_LFUNC_NARGS_VARARGS) {
			vs_min_regs = *out_nargs + DUK_VALSTACK_API_ENTRY_MINIMUM + DUK_VALSTACK_INTERNAL_EXTRA;
		} else {
			*out_nargs = -1;  /* vararg */
			vs_min_regs = num_stack_args + DUK_VALSTACK_API_ENTRY_MINIMUM + DUK_VALSTACK_INTERNAL_EXTRA;
		}
	...
	*out_vs_min_bytes = entry_valstack_bottom_byteoff + sizeof(duk_tval) * (idx_func + 2 + vs_min_regs);
...
DUK_LOCAL void duk__call_env_setup(duk_hthread *thr, duk_hobject *func, duk_activation *act, duk_idx_t num_stack_args) {
	duk_hobject *env;
 ...
 *
 *    - Tailcalls, where an activation is reused without increasint call
 *      stack (duk_activation) depth.
 *
 *    - Setup for an initial Duktape.Thread.resume().
 *
 *  The call handler doesn't provide any protection guarantees, safe calls
...
DUK_LOCAL duk_int_t duk__handle_call_raw(duk_hthread *thr,
	...
	DUK_DD(DUK_DDPRINT("duk__handle_call_raw: idx_func=%ld, call_flags=0x%08lx, top=%ld",
	                   (long) idx_func, (long) call_flags, (long) duk_get_top(ctx)));
	...
	num_stack_args = duk_get_top(ctx) - (idx_func + 2);  /* FIXME: check if even this can be removed below */
	...
	if (use_tailcall) {
		use_tailcall = duk__call_setup_act_attempt_tailcall(thr, call_flags, idx_func, func, entry_valstack_bottom_byteoff, &nargs, &nregs, &vs_min_bytes, &act);
	}
#else
	...
	if (!use_tailcall) {
		duk__call_setup_act_not_tailcall(thr, call_flags, num_stack_args, idx_func, func, entry_valstack_bottom_byteoff, &nargs, &nregs, &vs_min_bytes, &act);
	}
	...
	duk__call_env_setup(thr, func, act, num_stack_args);
	...
	if (func != NULL && DUK_HOBJECT_IS_COMPFUNC(func)) {
		...
		if (call_flags & DUK_CALL_FLAG_ALLOW_ECMATOECMA) {
			DUK_DPRINT("avoid native call, use existing executor");
			DUK_ASSERT((act->flags & DUK_ACT_FLAG_PREVENT_YIELD) == 0);
...
DUK_INTERNAL duk_int_t duk_handle_safe_call(duk_hthread *thr,
	...
	/* FIXME: entry reserve... what semantics if duk_safe_call() target code
	 * extends reserve???  keep the extended reserve?
	 */
...
DUK_LOCAL void duk__handle_safe_call_error(duk_hthread *thr,
	...
	/* thr->ptr_curr_pc and thr->state are restored by
	 * duk__handle_safe_call_shared() which is also used
	 * for success path.
	 */
}
...
DUK_LOCAL void duk__handle_safe_call_shared_unwind(duk_hthread *thr,
	...
	/* FIXME: move to error path only, depending on success path changes. */
	/* Restore entry thread executor curr_pc stack frame pointer. */
	thr->ptr_curr_pc = entry_ptr_curr_pc;
	 ...
	 * case in test-bug-multithread-valgrind.c, until call handling was
	 * reworked.  Change in error path?
	 */
	DUK_HEAP_SWITCH_THREAD(thr->heap, entry_curr_thread);  /* may be NULL */
	thr->state = (duk_uint8_t) entry_thread_state;
	...
	/* FIXME: thr->ptr_curr_pc, thr->state */
}
